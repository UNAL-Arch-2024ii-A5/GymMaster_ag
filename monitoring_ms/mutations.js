...
const machineMutations = {
  createMachine: async (_, { input }) => {
    try {
      const response = await axios.post(`${process.env.MONITORING_URL}/machine`, input);
  ...
  updateMachine: async (_, { id, input }) => {
    try {
      const response = await axios.put(`${process.env.MONITORING_URL}/machine/${id}`, input);
      return response.data;
    } catch (error) {
...
const monitoringMutations = {
  createMonitoringEvent: async (_, { input }) => {
    try {
      const response = await axios.post(`${process.env.MONITORING_URL}/monitoring`, input);
      return response.data;
    } catch (error) {
  ...
  closeMonitoringEvent: async (_, { id, input }) => {
    try {
      const response = await axios.put(`${process.env.MONITORING_URL}/monitoring/close/${id}`, input);
      return response.data;
    } catch (error) {
